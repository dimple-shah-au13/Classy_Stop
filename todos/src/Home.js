...
const Home = ({state, styles, ...actions}) => {
    ...
    const slice = state.notes_list.slice(state.offset, state.offset + state.perPage)
    ...
    return(
      <>
        ...
        <div className="popup" style={state.showPopUp ? styles.inputStyle : styles.inputStyle1}>
            ...
            <label>
          Set status:
          <select value={state.edited_note.status}  onChange={(e)=>actions.handleChangeNote(e.target.value,"status", "edited_note")} className="edit-input" >
        ...
        <ul>
          <Masonry
            ...
            {(!state.search ? slice : state.search_list).filter(n=>{ return ((n.id !== state.pinned_id) && (n.id !== state.popUp_id)) }).map((item,index)=>
              <li key={index} style={item.status==="InProgress" || item.status==="Backlog"?item.status==="InProgress"?{backgroundColor:"yellow"}:{backgroundColor:"red"}:{backgroundColor:"green"}}className="list-item" >
                <span className="span1">{item.tag}<button className="pin-button" onClick={e=>actions.pinNote(item.id)}><img className="pin" src="./push-pin.png"/></button></span>
                ...
                <button className="list-button" onClick={e=>actions.removeFromNotes(index, item.id)}>Delete</button>
                </div></li>)}
